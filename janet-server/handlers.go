...
func (s *Server) handleAPIRecentActivity(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.logger.Err(err).KV("query", query).Error("failed to get recent activity")
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			s.logger.Err(err).Error("failed to scan activity row")
			continue
		}
	...
	var totalCount int
	err = s.db.SQL.QueryRow(countQuery, countArgs...).Scan(&totalCount)
	if err != nil {
		s.logger.Err(err).KV("countQuery", countQuery).Error("failed to get activity count")
		totalCount = 0
	}
...
func (s *Server) handleAPIUserAllTimePointsOverTime(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	username := vars["username"]
	...
	rows, err := s.db.SQL.Query(query, username)
	if err != nil {
		s.logger.Err(err).Error("failed to get user all-time points over time")
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			s.logger.Err(err).Error("failed to scan yearly points")
			continue
		}
